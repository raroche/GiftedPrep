...
| **Chess Club** | Fifty-two lessons across three levels, one idea at a time, in the order every serious chess programme uses: rook first, knight late, checkmate last. Eight mini-games from Pawn Wars up to a whole game, five opponents that lose on purpose, and 3,250 puzzles filtered out of the six million in the CC0 Lichess database. Stars only ever go up, there is no streak to break, and taking a move back is always free. The room is loaded only when a child opens it |
